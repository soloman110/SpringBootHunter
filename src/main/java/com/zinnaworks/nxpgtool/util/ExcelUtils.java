...
//https://www.programcreek.com/java-api-examples/?api=org.apache.poi.ss.util.CellRangeAddress
...
public class ExcelUtils {
	...
	public static Cell findCellByValue(Sheet sheet, String value) throws Exception {
		Row firstrow = sheet.getRow(sheet.getFirstRowNum());
		int lastRowNum = sheet.getLastRowNum();
		int colFirstNum = firstrow.getFirstCellNum();
		int colLastNum = firstrow.getLastCellNum();
		for (int i = firstrow.getRowNum(); i < lastRowNum; i++) {
			Row row = sheet.getRow(i);
			for (int j = colFirstNum; j < colLastNum; j++) {
				Cell cell = row.getCell(j);
				String text = cell.getStringCellValue();
				if(!"".equals(text.trim()) && text.trim().equals(value)) {
					return cell;
				}
			}
		}
		throw new Exception("Sheet:" + sheet.getSheetName() + "에서 " +value + "를 못찾겠어요...");
	}
